<template>
	<div class="nav">
		<a v-for="(x, k) in navList" :key="k" class="nav-item"
			:class="[route.path.startsWith(x.path) && x.path ? 'current' : '']" :to="x.path" @click="go(x)">
			{{ x.name }}</a>
	</div>
</template>
...
.nav {
	...
	.nav-item {
		...
		background: url(https://cdn.molitao.top/20250330/e9yj5iue8yfio467ircsaf4klmrl3xl4.png) no-repeat center 3px / 100% 100%;
	}

	.nav-item.current {
		color: #b45000;
		background-image: url(https://cdn.molitao.top/20250330/gvqqcq5dq4actzr4j6ngygiobg4a5wct.png);
	}
}
</style>
