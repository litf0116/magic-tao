	...
	<div>
		<PagedTable ref="tableRef" :table-sort="'creationTime desc'" :fetch-function="fetchData"
			...
			<el-table-column label="是否置顶" prop="content" width="100">
				<template #default="{ row }">
					<el-switch v-model="row.isTop" active-color="#13ce66" inactive-color="#ff4949" :active-value="true"
						:inactive-value="false" @change="setIsTop(row)" />
				</template>
			</el-table-column>
			<el-table-column label="是否精华帖" prop="content" width="100">
				<template #default="{ row }">
					<el-switch v-model="row.isEssence" active-color="#13ce66" inactive-color="#ff4949"
						:active-value="true" :inactive-value="false" @change="setPostEssence(row)" />
				</template>
			</el-table-column>
