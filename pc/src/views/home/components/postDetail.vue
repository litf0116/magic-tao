		...
		<div class="post-detail">
			...
			<div class="main-content">
				...
				<div style="border-bottom: 1px dashed #D8D8D8;padding-bottom: 5px;">
					<el-tag v-if="postCategory" v-for="(item, index) in postCategory" :key="index"
						:type="tagTypes[index % tagTypes.length]" effect="plain" size="small" style="margin-left: 5px;">
						{{ item }}
					</el-tag>
				</div>
				<!-- 内容区域 -->
				<div class="content" v-html="post.content" @click="catchImage(post.content)"></div>
			</div>
			...
			<div class="side-info">
				...
				<el-button class="back-btn" type="primary" @click="adminSend(post)" plain>点击留言</el-button>
				<div style=" display: flex;justify-content:center;margin-top: 10px;">
					<el-button v-if="userStore.user.id === post.userId" class="edit-btn" type="primary"
						@click="editData(post)" plain>
						编辑
					</el-button>
					<el-button v-if="userStore.isAuctionAdmin || userStore.user.id === post.userId" class="edit-btn"
						type="danger" @click="delData(post)" plain>
						删除
					</el-button>
					<el-button v-if="userStore.isAuctionAdmin" class="edit-btn" type="primary" @click="setEssence(post)"
						plain>
						精华
					</el-button>
					<el-button v-if="userStore.isAuctionAdmin" class="edit-btn" type="primary" @click="setTop(post)"
						plain>
						置顶
					</el-button>
				</div>
