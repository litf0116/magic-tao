		...
		<div class="contact-left max-h-700px overflow-y-scroll">
			<template v-if="chatStore.friends0 && chatStore.friends0.length">
				...
				<div class="friend-list">
					<div v-for="(friend, idx) in chatStore.friends0" :key="idx" class="friend-item"
						...
						<div class="friend">
							...
							<div>
								<el-button size="small" type="success"
									@click="aggree(friend.id!, true)">同意</el-button><el-button size="small"
									@click="aggree(friend.id!, false)">拒绝</el-button>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="friend-list">
				<div class="h-8 mx-2">
					<el-input size="small" v-model="filterText" class="border-0" placeholder="筛选" clearable />
				</div>
				<div v-for="(friend, idx) in (chatStore.friends || []).filter((x) => x.name.indexOf(filterText) > -1)"
					:key="idx" class="friend-item"
					:class="{ actived: profile.friend && profile.friend.id === friend.id }"
