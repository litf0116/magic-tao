<template>
	<div ref="scrollView" class="chat-main">
		<div ref="messageList" class="message-list">
			...
			<div v-else class="my-2" :class="history.allLoaded ? 'history-loaded' : 'load'"
					...
					<div class="message-item-content" :class="{ self: message.from === userStore.user.id }">
						...
						<template v-else>
							<div class="sender-info" @click.right="showActionPopup(message)">
								<img v-if="userStore.user.id === message.from && message.fromTag != '拍卖师'"
							...
							<div class="message-content">
								<div @click="showGroupChatRules = true" class="message-fromName"
									...
									<div v-else-if="message.userChatLevel && isPrivateChat === false"
								...
								<div class="message-payload mt-1">
									<div v-if="message.type === ChatMessageType.Text" class="content-text"
										@click="showDetails(message)">
										<div v-html="renderTextMessage(message)"></div>
									</div>
									<!-- 图片消息 -->
									<div v-if="message.type === ChatMessageType.Image && message.payload"
										class="content-image" @click="showImagePreviewPopup(message)"
										@contextmenu.prevent="showActionPopup(message, true)">
										<img :data-url="getImgUrl(message, false)" :src="getImgUrl(message)"
									...
									<div v-if="message.type === ChatMessageType.AuctionStart && message.payload"
										...
										<div class="max-w-350px min-w-200px" @click="catchImage(message)"
											v-html="getStartContent(message)"></div>
									</div>
									<!-- 出价 -->
									<div v-if="message.type === ChatMessageType.AuctionBid && message.payload"
										@click="showDetails(message)" style="cursor: pointer;"
									...
									<div v-if="message.type === ChatMessageType.AuctionEnd && message.payload"
										...
										<div class="max-w-350px min-w-200px" @click="showDetails(message)"
	...
	<div class="chat-footer">
		<div class="action-box">
				...
				<el-popover v-model:visible="emojiVisible" placement="top-end" trigger="click" width="450">
					...
					<el-tabs type="card">
						<el-tab-pane label="系统">
							<img v-for="(emojiItem, emojiKey, index) in emoji.map" :key="index" class="emoji-item"
						...
						<el-tab-pane label="收藏">
							<div class="grid grid-cols-6 gap-2">
								<div v-for="(e, index) in emojiStore.userEmoji" :key="index" class="relative">
									<img class="w-full hover:scale-110 object-contain" :src="getImgUrl2(e.url, true)"
										@click="chooseUserEmoji(e)" />
									<i class="i-mdi:close-circle text-red-500 absolute top-0 right-0 cursor-pointer hover:scale-120"
			...
			<div class="input-box">
				<chat-input v-model="text" @focus="emoji.visible = false" @file-uploaded="sendImageMessage"
	...
	<div v-if="showGroupChatRules" class="action-popup" @click="actionPopup.visible = false">
		<div class="action-popup-main" style="width: 280px;padding: 10px;">
			...
			<div class="action-item" style="border-bottom: 0px solid #efefef;" @click="showGroupChatRules = false">取消
			</div>
		</div>
	</div>
