	...
	<view class="chatInterface" @contextmenu.prevent="">
		<view id="scrollview" class="scroll-view">
			...
			<view v-for="(message, index) in historyMsgs" :key="message.id">
				...
				<view class="message-item">
					<div v-if="message.type === ChatMessageType.Welcome"
						class="text-center flex-1 text-gray-600 text-sm">
						{{ message.fromName }} 加入群聊
					</div>
					<div v-else-if="message.type === ChatMessageType.BanUser"
						class="text-center flex-1 text-gray-600 text-sm">
						{{ message.msg }}
					</div>
					<div v-else-if="message.type === ChatMessageType.Backout"
						class="text-center flex-1 text-gray-600 text-sm">
						{{ message.msg }}
					</div>
					...
					<view v-else class="message-item-content" :class="{ self: message.from === userStore.user.id }">
						...
						<view class="content">
							<div @tap="showGroupChatRules = true" class="message-fromName" style="display:flex;"
								:class="[message.fromAdmin ? ' !text-red-500' : '']">
								<span v-if="message.fromAdmin && message.fromTag"
									:class="[message.tagClass ? message.tagClass : '']">{{
										message.fromTag }}
								</span>
								<div v-else class="tag_AuctionManager"
									:style="{ background: `linear-gradient(90deg,${message.userChatLevel.borderColor},${message.userChatLevel.rightBorderColor})` }"
							...
							<view class="message-payload mt-1">
								<view v-if="message.type === 'Text'" @tap="goShowDetails(message)"
									v-html="renderTextMessage(message)" :data-data="message"></view>
								<template v-if="message.type === 'Image'">
									<image class="image-content" mode="heightFix" :src="getImgUrl(message)" :style="{
										height:
											getImageHeight(message.payload.width, message.payload.height) + 'rpx',
									}" :data-url="getImgUrl(message, false)" @tap.stop="showImageFullScreen"
										@longpress.stop="showActionPopup(message, true)"></image>
								</template>

								<!-- 开始拍卖 -->
								<div v-if="message.type === ChatMessageType.AuctionStart && message.payload"
									...
									<div class="max-w-350px min-w-200px" :data-data="message" @tap="catchImage"
										v-html="getStartContent(message)"></div>
								</div>

								<!-- 出价 -->
								<div v-if="message.type === ChatMessageType.AuctionBid && message.payload"
									...
									<div class="max-w-350px min-w-200px" :data-data="message"
										v-html="getBitContent(message)"></div>
								</div>

								<!-- 公布得主 -->
								<div v-if="message.type === ChatMessageType.AuctionEnd && message.payload"
									...
									<!-- 
                                    <div class="max-w-350px min-w-200px" :data-data="message" @tap="catchImage"
                                        v-html="getEndContent(message)"></div> -->
									<div class="max-w-350px min-w-200px" @tap="showDetails(message)"
										v-html="getEndContent(message)"></div>
								</div>
							</view>
						</view>
		...
		<view class="action-box">
			<view class="action-top">
				<!-- <view @click="switchAudioKeyboard">
                    <image v-if="audio.visible" class="more" src="/static/images/jianpan.png"></image>
				...
				<input v-model="text" class="consult-input" maxlength="700" placeholder="发送消息" type="text"
			...
			<view v-if="emoji.visible" class="action-bottom action-bottom-emoji">
				<view class="flex flex-col">
					<view class="flex items-center">
						<view class="px-4 py-1" :class="[emojiIndex === 0 ? 'bg-#f4835a text-white' : ' text-gray-600']"
							@click="emojiIndex = 0">系统</view>
						<view class="px-4 py-1" :class="[emojiIndex === 1 ? 'bg-#f4835a text-white' : ' text-gray-600']"
							@click="emojiIndex = 1">收藏</view>
					</view>
					<view class="max-h-300rpx overflow-y-scroll pb-8" v-if="emojiIndex === 0">
						<image v-for="(emojiItem, key, index) in emoji.map" :key="index" class="emoji-item"
							:src="emoji.url + emojiItem" @click="chooseEmoji(key)"></image>
					</view>
					<view v-else class="max-h-300rpx overflow-y-scroll pb-8">
						<image v-for="(emojiItem, index) in emojiStore.userEmoji" :key="index" class="!size-16 mr-2"
							mode="aspectFit" :src="getImgUrl2(emojiItem.url, true)"
							:data-url="getImgUrl2(emojiItem.url, false)" @tap.stop="chooseUserEmoji(emojiItem)"
							@longpress.stop="emojiStore.removeEmoji(emojiItem)"></image>
					</view>
				</view>
			</view>
			<!--其他类型消息面板-->
		...
		<view v-if="actionPopup.visible" class="action-popup" @touchmove.stop.prevent>
			...
			<view class="action-list">
				...
				<div v-if="userStore.isChatAdmin || (selectMessage?.from && selectMessage.from === userStore.user.id)"
					class="action-item" @click="backout()">
					撤销
				</div>
				<div v-if="
				...
				" class="action-item" @click="adminSend()">
					私聊
				</div>
				<div v-if="selectMessage?.from && selectMessage.from !== userStore.user.id" class="action-item"
					@click="addFriend()">
					加为好友
				</div>
				<div v-if="selectMessage?.from && selectMessage.from !== userStore.user.id" class="action-item"
			...
			<view v-if="showItem" class="p-4">
				<view class="flex flex-row items-center overflow-hidden cursor-pointer">
					<view class="text-wrap px-2 flex-1 flex flex-col">
						...
						<view @tap="catchImage(showItem)"
							v-html="showItem.description === undefined ? showItem.Description : showItem.description">
						</view>
					</view>
				</view>
			</view>
...
function renderTextMessage(message: ChatMessage) {
	if (message && message.msg) {
		return (
			'<span class="text-content">' + emoji.value.decoder.decode(message.msg.replaceAll('\n', '<br/>')) + '</span>'
		)
	}
...
function getEndContent(message: ChatMessage) {
	...
	if (message.payload.status === '已成交') {
		return `<div class="text-red-500">恭喜 ${message.payload.dealUserName} 最终以 <b class="text-lg">￥${message.payload.finalPrice
			}</b> 拍得商品</div>
...
function sendImageMessage2() {
	...
	//         console.log('sendImageMessage2', res, typeof res)
	//         const files = res.tempFiles as ChooseImageSuccessCallbackResultFile[]
	//         if (files.length === 0) return
	//         files.forEach((file) => {
	//             console.log('sendImageMessage2', file)
	//         })
	//     },
	// })
