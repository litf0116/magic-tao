<template>
	<view :class="{ kongjiang: checkIncludes }" class="shadow flex flex-row items-center overflow-hidden cursor-pointer"
		@click.stop="showDetail(item)">
		<div class="text-[#935F4E] line-clamp-3" style="margin-right: 5px;"
			v-if="checkIncludes && item.status === '上架'">{{ index }}</div>
