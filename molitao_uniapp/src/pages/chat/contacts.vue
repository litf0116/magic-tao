<template>
	<view class="contacts">
		<scroll-view class="scroll-container" scroll-y="true" refresher-enabled="true" :refresher-triggered="refresh"
			@refresherrefresh="onRefresh" :enhanced="true" :bounce="true" :show-scrollbar="true" fast-deceleration>
			<view class="flex items-center bg-white">
				<view class="flex-1 m-2">
					<uv-input ref="searchInput" v-model="filterText" shape="circle" placeholder="搜索联系人"
						prefixIcon="search" prefixIconStyle="font-size: 22px;color: #909399"></uv-input>
				</view>
			</view>
			<view class="contacts-container">
					...
					<view class="user-list">
						<view v-for="friend in chatStore.friends0" :key="friend.id" class="user-list-item">
							...
							<view class="flex mr-2 space-x-1">
								<uv-button type="success" size="small" text="同意"
									@click="agree(friend.id!, true)"></uv-button>
								<uv-button type="default" :plain="true" size="small" text="拒绝"
									@click="agree(friend.id!, false)"></uv-button>
							</view>
						</view>
					</view>
				</template>
				...
				<view class="user-list">
					<view v-for="friend in (chatStore.friends || []).filter((x) => x.name.indexOf(filterText) > -1)"
						:key="friend.id" class="user-list-item" @click="privateChat(friend)">
						<view class="user-item-avatar">
							<image :src="getImgUrl(friend.headImgUrl!, true)" mode="aspectFill"></image>
						</view>
						<view class="user-item-info">
