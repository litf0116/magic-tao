	...
	<view>
		<view v-if="item" class="sticky top-0 left-0 right-0 z-99">
			<uv-notice-bar mode="link" bgColor="#f4835a" color="#ffffff" :text="announceContent"
				:url="`/pages/announce/list?id=1`"></uv-notice-bar>
		</view>
		...
		<uv-popup ref="popupRef" mode="right">
			<view class="w-65vw h-100vh">
				<div class="shadow h-10 grid grid-cols-3">
					...
					<div v-if="myGroup" class="col-span-2 bg-orange-300 text-white cursor-pointer flex flex-center"
						@click="deleteMyGroup">
						删除我的组队频道
					</div>
					<div v-else class="col-span-2 bg-[#f4835a] text-white cursor-pointer flex flex-center"
						@click="createGroup">
						创建组队聊天
					</div>
				</div>
				<div class="space-y-2 overflow-y-scroll" style="height: calc(100vh - 80rpx)">
					<view class="p-2">
						<div v-for="(group, k) in chatStore.groups
							.filter(
								(x) =>
									x.chan !== '0_lobby' &&
									x.chan !== '-1_auction' &&
							...
							.reverse()" :key="k">
							<div class="px-2 rounded h-8 flex flex-center text-true-gray-8 shadow-md cursor-pointer relative text-sm flex items-center justify-between"
								:class="[
