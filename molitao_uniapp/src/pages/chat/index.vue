<template>
	<scroll-view class="conversations" scroll-y="true">
		<view v-if="chatStore.chatList.length">
			<view v-for="(x, key) in orderBy(chatStore.chatList, ['order'], ['desc'])" :key="key" class="scroll-item"
				@tap.stop="chat(x)">
				<view class="item-head">
					...
					<template v-else>
						...
						<div v-else-if="x.id === -1"
							class="head-icon bg-green-600 text-white font-bold w-full h-full flex flex-center">
							拍卖
						</div>
						<div v-else class="head-icon bg-black text-white font-bold w-full h-full flex flex-center"
				...
				<view class="scroll-item_info">
					<view class="item-info-top">
						<text class="item-info-top_name" :class="[
							x.name === 'lobby'
