...
TOKEN_7509=$(curl -s -X POST "$BASE_URL/api/TokenAuth/GenerateTokenForUser" \
  -H "Content-Type: application/json" \
...
## 八、测试结果（2026-04-24 实际测试）

### 8.1 已验证用例

| 用例ID | 描述 | 测试结果 | 备注 |
|--------|------|---------|------|
| TC-005 | 查看已同意好友列表 | ✅ 通过 | 用户14和7509互为双向好友 |
| TC-011 | 自己给自己添加好友 | ✅ 通过 | 正确跳过，无任何操作 |
| TC-003 | 添加已是好友的用户 | ✅ 通过 | 返回"对方已是你的好友" |
| TC-006 | 获取待处理好友请求数量 | ✅ 通过 | count=0（无待处理请求） |
| TC-017 | 获取好友数量为0 | ✅ 通过 | 正确返回{count:0} |

### 8.2 业务规则验证

1. **好友申请单向性**：申请方A → 接收方B（UserId=B, FriendId=A）
2. **双向好友关系**：Agree后自动建立双向记录
3. **防重复机制**：已存在好友关系时拒绝添加
4. **无自添加**：自己给自己发送请求时跳过（代码中if判断）
