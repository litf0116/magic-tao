    ...
    public static class MapConverter
    {
        ...
        public static void GCJ02ToWGS84Exact(double mgLat, double mgLon, out double wgLat, out double wgLon)
        {
            ...
            do
            {
                ...
                if (dLat > 0) pLat = wgLat;
                else mLat = wgLat;
                if (dLon > 0) pLon = wgLon;
                else mLon = wgLon;
            } while (++i <= 30);
        }
        ...
        public static double Distance(this (double LatA, double LonA) a, (double LatB, double LonB) b)
        {
            ...
            double x = Math.Cos(a.LatA * PI / 180.0) * Math.Cos(b.LatB * PI / 180.0) * Math.Cos((a.LonA - b.LonB) * PI / 180);
            double y = Math.Sin(a.LatA * PI / 180.0) * Math.Sin(b.LatB * PI / 180.0);
            double s = x + y;
            if (s > 1) s = 1;
            if (s < -1) s = -1;
