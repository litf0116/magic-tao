...
using TtWork.HttpClient.Weixin.WeixiinResult.TenPay;
using TtWork.Lib;
...
namespace TtWork.HttpClient.Weixin
{
    public interface IPayApi
    {
        ...
        Task<dynamic> RefundAsync(string appId, string mchId, string mchKey, string subAppId, string subMchId,
            string transactionId, string outTradeNo, string outRefundNo,
            int totalFee, int refundFee, string certPath, string refundFeeType = "CNY", string refundDesc = "",
            string refundAccount = "REFUND_SOURCE_UNSETTLED_FUNDS",
            string notifyUrl = "");

        Task<TransfersResult> Transfers(string mch_appid, string mchid, string mchKey, string partner_trade_no,
        ...
        : IPayApi
    {
            ...
            string limitPay = "", string subOpenId = "", string sceneInfo = "")
        {
            ...
            var signStr = signatureGenerator.Generate(request, MD5.Create(), mchKey);
            ...
            result.PaySign = signatureGenerator.GetJsPaySign(
        ...
        /// <param name="refundAccount">仅针对老资金流商户使用，具体参考 <see cref="RefundAccountType" /> 的定义。</param>
        /// <param name="notifyUrl">异步接收微信支付退款结果通知的回调地址，通知 Url 必须为外网可访问的 Url，不允许带参数。如果传递了该参数，则商户平台上配置的回调地址将不会生效。</param>
        public async Task<dynamic> RefundAsync(string appId, string mchId, string mchKey, string subAppId, string subMchId,
            string transactionId, string outTradeNo, string outRefundNo,
            int totalFee, int refundFee, string certPath, string refundFeeType = "CNY", string refundDesc = "",
            string refundAccount = "REFUND_SOURCE_UNSETTLED_FUNDS",
            string notifyUrl = "")
        {
            try
            {
                ...
                var signStr = signatureGenerator.Generate(request, MD5.Create(), mchKey);
                request.AddParameter("sign", signStr);
                var xmlResult = await RequestAndGetReturnValueAsync("https://api.mch.weixin.qq.com/secapi/pay/refund", request, certPath, mchId);
                var returnCode = GetXmlNodeString(xmlResult, "return_code");
                var resultode = GetXmlNodeString(xmlResult, "result_code");
                var result = Newtonsoft.Json.JsonConvert.SerializeObject(xmlResult);
                logger.LogWarning("退款结果:{result}", result);
                if (returnCode == "SUCCESS" && resultode == "SUCCESS") return await Task.FromResult(result);
                throw new Exception(GetXmlNodeString(xmlResult, "err_code_des"));
            }
            catch (Exception e)
            {
            ...
            string transactionId, string outTradeNo)
        {
            ...
            var signStr = signatureGenerator.Generate(request, MD5.Create());
            request.AddParameter("sign", signStr);
        ...
        protected virtual async Task<XDocument> RequestAndGetReturnValueAsync(string targetUrl,
            PayParameters requestParameters, string certPath = "",string mchId="")
        {
            var result = await RequestAsync(targetUrl, requestParameters.ToXmlStr(), certPath, mchId);
            ...
            logger.LogInformation("{@TenPay_XmlResult}", result.ToString());
            return result;
        }
        ...
        /// <returns></returns>
        /// 
        public async Task<TransfersResult> Transfers(string mch_appid, string mchid, string mchKey,
            string partner_trade_no,
            string openid, string re_user_name, int amount, string desc, bool check_name = true)
        {
            var request = new PayParameters();
            request.AddParameter("mch_appid", mch_appid); ;//公众账号appid， 注意是mch_appid，而非wxappid
            request.AddParameter("mchid", mchid);//商户号, 注意是mchid而非mch_id
            request.AddParameter("nonce_str", RandomExt.GetRandom());//随机字符串
            ...
            var signStr = signatureGenerator.Generate(request, MD5.Create(), mchKey);
            request.AddParameter("sign", signStr);
            var xmlResult = await RequestAndGetReturnValueAsync("mmpaymkttransfers/promotion/transfers", request);

            var result = new TransfersResult()
            {
                return_code = GetXmlNodeString(xmlResult, "return_code"),
                mch_appid = GetXmlNodeString(xmlResult, "mch_appid"),
                mchid = GetXmlNodeString(xmlResult, "mchid"),
                result_code = GetXmlNodeString(xmlResult, "result_code"),
                partner_trade_no = GetXmlNodeString(xmlResult, "partner_trade_no"),
                payment_no = GetXmlNodeString(xmlResult, "payment_no"),
                payment_time = GetXmlNodeString(xmlResult, "payment_time"),
                err_code = GetXmlNodeString(xmlResult, "err_code"),
                err_code_des = GetXmlNodeString(xmlResult, "err_code_des"),
            };
        ...
        public async Task<XDocument> RequestAsync(string url, string body, string certPath, string mchid)
        {
            //创建HttpClientHandler并设置证书
            HttpClientHandler handler = new HttpClientHandler();
            X509Certificate2 certificate = new X509Certificate2(certPath, mchid);
            handler.ClientCertificates.Add(certificate);
            //创建httpclient 并设置HttpClientHandler
            using (System.Net.Http.HttpClient httpClient = new System.Net.Http.HttpClient(handler))
            {
                //发送请求
                System.Net.Http.HttpContent content = new StringContent(body, Encoding.UTF8, "application/xml");
