...
namespace TtWork.Abp.Authorization.Users {
    public class UserManager : AbpUserManager<Role, User> {
        ...
        public virtual async Task<List<User>> GetUsersInOrganizationUnit(
            long organizationUnitId,
            bool includeChildren = false) {
            var organizationUnit = await _organizationUnitRepository.FirstOrDefaultAsync(organizationUnitId);
            return await GetUsersInOrganizationUnitAsync(organizationUnit, includeChildren);
        }
        ...
        private void CheckPermissionsToUpdate(User user, IEnumerable<Permission> permissions) {
            if (user.Name == AbpUserBase.AdminUserName &&
                (!permissions.Any(p => p.Name == AppPermissions.Administration) ||
                 !permissions.Any(p => p.Name == AppPermissions.Administration))) {
