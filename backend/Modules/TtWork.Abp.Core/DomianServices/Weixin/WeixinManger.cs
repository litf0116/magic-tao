...
namespace TtWork.Abp.DomianServices.Weixin
{
        ...
        IIocManager iocManager) : AppDomainServicebase(iocManager)
    {
                ...
                if (ttl.HasValue && ttl.Value.TotalSeconds < 300)
                {
                    _ = Task.Run(async () =>
                    {
                        try
                        {
                            ...
                            {
                                await redisClient.Database.StringSetAsync(key, newToken.access_token,
        ...
        public virtual async Task<BaseWeChatReulst> ImgSecCheck(byte[] imageBuffer)
        {
            var accessToken = await GetAccessTokenAsync();
            return await weixinApi.ImgSecCheck(accessToken, imageBuffer);
        }
    }
}