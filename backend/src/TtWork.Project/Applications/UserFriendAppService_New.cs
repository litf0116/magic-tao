...
public class UserFriendAppService(
    ...
    /// </summary>
    [HttpGet]
    public async Task<ListResultDto<UserDtoBase>> GetMyFriends() {
        var userId = AbpSession.UserId!.Value;
        
        // 获取所有涉及该用户的已接受好友关系（双向查询）
    ...
    /// </summary>
    [HttpGet]
    public async Task<ListResultDto<UserDtoBase>> GetPendingFriendRequests() {
        var userId = AbpSession.UserId!.Value;
        
        // 获取发送给用户的好友请求（用户是接收方）
        var pendingRequests = await repository.GetAll().AsNoTracking()
            .Where(x => x.FriendId == userId && x.Status == false)
            .ToListAsync();
            
        List<UserDtoBase> result = [];
        foreach (var request in pendingRequests) {
            var cache = await userCache.GetAsync(request.UserId);
            if (cache != null)
                result.Add(ObjectMapper.Map<UserDtoBase>(cache));
        }
