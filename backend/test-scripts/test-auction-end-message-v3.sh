...
# 预设用户
ADMIN_ID=14        # 拍卖师/管理员用户 ID
USER_ID=7509       # 测试用户 ID
...
get_user_token() {
    ...
    # 提取 token
    local token=$(echo $response | python3 -c "import json,sys; print(json.load(sys.stdin).get('result',{}).get('accessToken',''))" 2>/dev/null)
...
# 步骤1：获取管理员 token
print_header "步骤1：获取管理员 token"

ADMIN_TOKEN=$(get_user_token $ADMIN_ID "管理员/拍卖师")

if [ $? -ne 0 ]; then
    print_error "无法获取管理员 token，测试终止"
    exit 1
fi
...
USER_ID=$(echo $USER_RESPONSE | python3 -c "import json,sys; data=json.load(sys.stdin); print(data.get('result',{}).get('user',{}).get('id',''))" 2>/dev/null)
USER_NAME=$(echo $USER_RESPONSE | python3 -c "import json,sys; data=json.load(sys.stdin); print(data.get('result',{}).get('user',{}).get('name',''))" 2>/dev/null)
...
# 查找一个可测试的拍卖商品 ID
AUCTION_ITEM_ID=$(echo "$AUCTION_LIST_RESPONSE" | python3 -c "import json,sys; data=json.load(sys.stdin); items=data.get('result',{}).get('items',[]); print(items[0]['id'] if items else '')" 2>/dev/null)
...
if [ -z "$AUCTION_ITEM_ID" ]; then
    ...
    AUCTION_ITEM_ID=$(echo "$CREATE_RESPONSE" | python3 -c "import json,sys; data=json.load(sys.stdin); print(data.get('result',{}).get('id',''))" 2>/dev/null)
...
START_SUCCESS=$(echo "$START_AUCTION_RESPONSE" | python3 -c "import json,sys; print(json.load(sys.stdin).get('success',False))" 2>/dev/null)
...
USER_TOKEN=$(get_user_token $USER_ID "测试用户")
...
if [ "$HAS_BIDDER" = true ]; then
    ...
    CURRENT_PRICE=$(echo "$BID_RESPONSE" | python3 -c "import json,sys; data=json.load(sys.stdin); print(data.get('result',{}).get('currentPrice',''))" 2>/dev/null)
...
# 验证拍卖成功消息
FINAL_PRICE=$(echo "$END_AUCTION_RESPONSE" | python3 -c "import json,sys; data=json.load(sys.stdin); print(data.get('result',{}).get('finalPrice',''))" 2>/dev/null)
DEAL_USER_ID=$(echo "$END_AUCTION_RESPONSE" | python3 -c "import json,sys; data=json.load(sys.stdin); print(data.get('result',{}).get('dealUserId',''))" 2>/dev/null)
DEAL_USER_NAME=$(echo "$END_AUCTION_RESPONSE" | python3 -c "import json,sys; data=json.load(sys.stdin); print(data.get('result',{}).get('dealUserName',''))" 2>/dev/null)

if [ -n "$FINAL_PRICE" ] && [ "$FINAL_PRICE" != "None" ]; then
...
items = data.get('result',[]) if 'result' in data else []
for msg in items[:3]:
...
# 步骤9：检查用户私聊（如果成交）
if [ -n "$DEAL_USER_ID" ] && [ "$DEAL_USER_ID" != "None" ]; then
...
echo "  1. 使用 GenerateTokenForUser 获取管理员 token (用户ID: $ADMIN_ID) -> ADMIN_TOKEN"
echo "  2. 使用 GenerateTokenForUser 获取测试用户 token (用户ID: $USER_ID) -> USER_TOKEN"
echo "  3. 获取拍卖商品列表"
echo "  4. 开始拍卖（管理员操作）"
echo "  5. 模拟出价（测试用户操作）"
echo "  6. 结束拍卖（管理员操作）"
...
echo ""
echo -e "${BLUE}📝 注意事项：${NC}"
echo "  - ADMIN_TOKEN: 管理员/拍卖师的访问令牌"
echo "  - USER_TOKEN: 普通测试用户的访问令牌"
echo "  - 拍卖成功会发送两条消息："
...
echo "  # 获取管理员 token (ADMIN_TOKEN)"
echo "  curl -X POST '${BASE_URL}/api/TokenAuth/GenerateTokenForUser' \\"
echo "    -H 'Content-Type: application/json' \\"
echo "    -d '{\"userId\": $ADMIN_ID}' | jq '.result.accessToken'"
echo ""
echo "  # 获取测试用户 token (USER_TOKEN)"
echo "  curl -X POST '${BASE_URL}/api/TokenAuth/GenerateTokenForUser' \\"
echo "    -H 'Content-Type: application/json' \\"
echo "    -d '{\"userId\": $USER_ID}' | jq '.result.accessToken'"